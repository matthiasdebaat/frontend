...
import { getCloudTTSInfo } from "../../../../data/cloud/tts";
...
import { getCloudTtsSupportedVoices } from "./cloud-tts-pref";
...
export class CloudAccount extends SubscribeMixin(LitElement) {
  ...
  @state() private _ttsInfo?: CloudTTSInfo;
  ...
  protected render() {
    ...
    const webhookCount = this._webhooks
      ? this._webhooks.filter(
          (hook) => !hook.local_only && cloudhooks[hook.webhook_id]
        ).length
      : 0;

    const defaultTtsLanguage = this.cloudStatus.prefs.tts_default_voice[0];
    const voiceCount = this._ttsInfo
      ? getCloudTtsSupportedVoices(defaultTtsLanguage, this._ttsInfo).length
      : undefined;
    const voiceLanguage = voiceCount
      ? formatLanguageCode(defaultTtsLanguage, this.hass.locale)
      : undefined;
    ...
    return html`
      ...
      >
        ...
        <div class="content">
          <ha-card outlined>
            <div
              class="card-header thank-you-header"
              role="heading"
              aria-level="1"
                ...
                <ha-md-list-item>
                  ...
                  <span slot="headline">
                    ${this._subscription?.plan_renewal_date
                      ? this.hass.localize(
                          "ui.panel.config.cloud.account.renews_on",
                          {
                            date: formatDate(
                              new Date(
          ...
          <ha-card outlined>
            <div class="extras-header">
              <h1 class="card-header">
                ${this.hass.localize(
                  "ui.panel.config.cloud.account.features.title"
                )}
              </h1>
              <img
                .src=${brandsUrl({
            ...
            <div class="card-content">
              <p>
                ${this.hass.localize(
                  "ui.panel.config.cloud.account.features.introduction"
                )}
              </p>
              <ha-md-list>
                <ha-md-list-item type="link" href="/config/cloud/remote">
                  <ha-svg-icon slot="start" .path=${mdiEarth}></ha-svg-icon>
                  <div slot="headline">
                    ${this.hass.localize(
                ...
                <ha-md-list-item type="link" href="/config/cloud/backup">
                  <ha-svg-icon
                    slot="start"
                  ...
                  <div slot="supporting-text">
                    ${this._backupConfig
                      ? this._backupConfig.last_completed_automatic_backup
                        ? this.hass.localize(
                            "ui.panel.config.cloud.account.backup.last_backup",
                            {
                        ...
                        : this.hass.localize(
                            "ui.panel.config.cloud.account.backup.no_backups"
                          )
                      : this.hass.localize(
                          "ui.panel.config.cloud.account.backup.loading"
                        )}
                  </div>
                  <ha-icon-next slot="end"></ha-icon-next>
                </ha-md-list-item>
                ...
                <ha-md-list-item
                  type="link"
                  href="/config/cloud/voice-assistants"
                >
                  <ha-svg-icon
                    slot="start"
                    .path=${mdiMicrophone}
                  ...
                  <div slot="headline">
                    ${this.hass.localize(
                      "ui.panel.config.cloud.account.tts.title"
                    )}
                  </div>
                  <div slot="supporting-text">
                    ${voiceCount
                      ? this.hass.localize(
                          "ui.panel.config.cloud.account.tts.voices_count",
                          {
                            count: voiceCount,
                ...
                <ha-md-list-item type="link" href="/config/cloud/google-alexa">
                  <ha-svg-icon
                    slot="start"
                ...
                <ha-md-list-item type="link" href="/config/cloud/webrtc">
                  <ha-svg-icon slot="start" .path=${mdiVideo}></ha-svg-icon>
                  <div slot="headline">
                    ${this.hass.localize(
                ...
                <ha-md-list-item type="link" href="/config/cloud/webhooks">
                  <ha-svg-icon slot="start" .path=${mdiWebhook}></ha-svg-icon>
                  <div slot="headline">
                    ${this.hass.localize(
                  ...
                  <div slot="supporting-text">
                    ${this.hass.localize(
                      "ui.panel.config.cloud.account.webhooks.active_count",
                      { count: webhookCount }
                    )}
                  </div>
                  <ha-icon-next slot="end"></ha-icon-next>
  ...
  private async _fetchTtsInfo() {
    this._ttsInfo = await getCloudTTSInfo(this.hass);
  }
  ...
  private async _fetchBackupConfig() {
    try {
      ...
      // Backup component may not be loaded
    }
  }
